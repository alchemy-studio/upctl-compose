...
\title{\textbf{upctl-compose 用户手册}\\[2pt]{\normalsize 本地部署 \& 全链路 E2E 测试指南}}
\subtitle{\small 基于 Docker Compose 的工单管理系统 -- 从部署到自动化测试}
\author{鉬馨玥翼}
\date{\today}
...
\begin{frame}[shrink=10]{系统架构总览}
  \begin{center}
    \begin{tikzpicture}[
      ...
      \node[draw, dashed, rounded corners, minimum width=1.8cm, minimum height=0.4cm, font=\tiny] (ds) at (0, -2) {DeepSeek};
      ...
      \node[font=\tiny, right] at (nginx.east) {:8088};
      \node[font=\tiny, right] at (auth.east) {:3000};
      \node[font=\tiny, right] at (svc.east) {:3005};
      \node[font=\tiny, right] at (gitea.east) {:3001};
    \end{tikzpicture}
  \end{center}
  \vspace{2pt}
  \begin{table}
...
\begin{frame}[shrink=10]{AI Agent 工作流程}
  \begin{center}
    \begin{tikzpicture}[node distance=0.5cm, auto]
      ...
      \node[block, fill=orange!20, right=of label] (llm) {DeepSeek};
  ...
  \begin{exampleblock}{关键特征}
    \footnotesize
    每 5 分钟轮询 {\small |} tmux 运行时 {\small |} 自动评论回复
  \end{exampleblock}
\end{frame}
...
\begin{frame}{登录页面}
  \begin{columns}[T]
    ...
    \begin{itemize}
      \item 开发模式显示 union\_id 输入框
      \item 输入任意值生成 JWT
      \item 登录后跳转工单列表
    \end{itemize}
    \vspace{4pt}
    \textcolor{myblue}{认证流程：} 前端 $\to$ nginx $\to$ authcore $\to$ JWT
...
\begin{frame}[fragile]{各组件日志解读}
  \begin{columns}[T]
    ...
    \begin{block}{ai-agent 处理}
      \tiny
      Processing issue \#18: test\\
      add\_label \#18: 200\\
      DeepSeek response (18 chars)\\
      add\_comment \#18: 200\\
      close \#18: 200 -- completed
    \end{block}
  \end{columns}
\end{frame}
...
\begin{frame}{E2E 测试概览}
  \small
  \begin{columns}[T]
    \column{0.48\textwidth}
    \textbf{后端测试} (tests/e2e\_test.py)
    \begin{itemize}
      \item 认证 (401 拒绝)
      \item Gitea 连通性
      \item 工单 CRUD
    ...
    \column{0.48\textwidth}
    \textbf{前端测试} (tests/playwright/)
    \begin{itemize}
      \item 登录页渲染
      \item 未认证重定向
      \item 工单列表
      \item 全流程管道 (pipeline.spec.ts)
    \end{itemize}
  \end{columns}
  \vspace{4pt}
  \textcolor{myred}{核心：} 创建 approved 工单 $\to$ ai-agent 处理 $\to$ 评论 $\to$ 关闭 $\to$ 浏览器验证
\end{frame}

\begin{frame}[fragile]{Playwright 全流程测试}
  \small
  \texttt{pipeline.spec.ts} 6 步模拟真实操作：
  \begin{enumerate}
    \item 生成 JWT $\to$ 注入浏览器 localStorage
    \item fetch 创建 approved 工单
    \item 导航详情页，验证待处理
    \item 触发 pick\_and\_process()
    \item 轮询 API 等待 closed
    \item 刷新验证已关闭 + 评论
  \end{enumerate}
  \vspace{4pt}
  特性：超时 5 分钟 {\small |} 浏览器原生 fetch {\small |} 无 API Key 时跳过
\end{frame}

\begin{frame}[fragile]{运行 E2E 测试}
  \begin{columns}[T]
    \column{0.48\textwidth}
    \begin{block}{后端}
      \tiny
      docker compose exec ai-agent \\
      \relax\ \ python3 /app/e2e\_test.py
    \end{block}
    \column{0.48\textwidth}
    \begin{block}{Playwright}
      \tiny
  ...
  \begin{exampleblock}{执行结果}
    ...
    \$ npx playwright test\\
    5 passed (9.8s)\\
    \relax\ \checkmark\ redirects to /login (45ms)\\
    \relax\ \checkmark\ renders title (44ms)\\
    \relax\ \checkmark\ Full pipeline (9.2s)
  \end{exampleblock}
\end{frame}
...
\begin{frame}[fragile]{ai-agent 工作日志}
  \begin{columns}[T]
    ...
    \begin{block}{处理工单}
      ...
      Processing issue \#18: ...\\
      add\_label \#18: 200\\
      DeepSeek response received (XX chars)\\
      add\_comment \#18: 200\\
      close \#18: 200 -- completed
    \end{block}
  \end{columns}
  \vspace{4pt}
  \textcolor{myblue}{关键技术：} Python 轮询 + tmux session + Gitea API (Basic Auth) + DeepSeek API
\end{frame}
...
\begin{frame}{总结}
  \begin{center}
    ...
    \vspace{0.2cm}
    {\normalsize Docker Compose 一键部署的工单管理系统}
