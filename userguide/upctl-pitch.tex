...
\begin{tikzpicture}[
  ...
  \draw[dashed, rounded corners, thick, gray] (-8.5, 3) rectangle (8.5, -3);
  ...
  \node[svc, fill=gray!15] (redis) at (6, -1) {redis};

  % Row 4: data / workers
  \node[svc, fill=yellow!15] (pg) at (-5.5, -2.5) {PostgreSQL};
  \node[svc, fill=red!15] (ai) at (2.5, -2.5) {ai-agent};
  \node[draw, thick, rounded corners, fill=cyan!10, font=\tiny] (ds) at (6, -2.5) {DeepSeek API};
  ...
  \draw[->,thick] (svc) -- (1.5, -1) -- (6, -1) -- (redis);
  ...
  % Arrows — ai ↔ svc, ai → ds
  \draw[->,thick] (ai) -- (0.5, -2.5) |- (svc);
  \draw[->,thick] (ai) -- (ds);

  % Port labels
  \node[font=\tiny, right] at ([xshift=0.3cm]nginx.east) {:8088};
  \node[font=\tiny, right] at ([xshift=0.3cm]auth.east) {:3000};
  \node[font=\tiny, right] at ([xshift=0.3cm]svc.east) {:3005};
  \node[font=\tiny, right] at ([xshift=0.3cm]gitea.east) {:3001};
  \node[font=\tiny, right] at ([xshift=0.3cm]redis.east) {:6379};
  \node[font=\tiny, right] at ([xshift=0.3cm]pg.east) {:5432};
\end{tikzpicture}
\end{center}
...
\begin{frame}[shrink=10]{AI Agent 工作流程}
  \begin{center}
    \begin{tikzpicture}[node distance=0.3cm, auto, font=\tiny]
      ...
      \draw[->, thick] ([yshift=0.4cm]ai.east) -- node[above] {每 5m 轮询} ([yshift=0.4cm]svc.west);
      \draw[->, thick] ([yshift=0.4cm]svc.east) -- node[above] {查 approved 工单} ([yshift=0.4cm]gitea.west);

      % Row 1 return (midline): dashed polling loop back
      \draw[->, thick, dashed] ([yshift=0cm]gitea.west) -- ([yshift=0cm]svc.east);
      \draw[->, thick, dashed] ([yshift=0cm]svc.west) -- ([yshift=0cm]ai.east);

      % Row 2 (bottom offset -0.4): processing forward
      \draw[->, thick] ([yshift=-0.4cm]ai.east) -- node[below] {加 in\_progress} ([yshift=-0.4cm]svc.west);
