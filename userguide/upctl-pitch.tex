...
\begin{frame}[shrink=3]{产品形态：三层架构}
  \begin{center}
    \begin{tikzpicture}[
      sbox/.style={rectangle, draw, rounded corners, fill=white, minimum width=2.2cm, minimum height=0.55cm, align=center, font=\tiny},
      scale=0.92
    ]

    % --- 适配层 ---
    \draw[rounded corners, fill=green!10, draw=black] (-5.8, 1.6) rectangle (5.8, 3.2);
    \node[font=\footnotesize\bfseries] at (0, 2.95) {适配层 (MCP / API)};
    \node[sbox] at (-3.6, 2.3) {Claude Code};
    \node[sbox] at (0, 2.3) {OpenAI GPT-4};
    \node[sbox] at (3.6, 2.3) {Local Model};

    % --- 引擎层 ---
    \draw[rounded corners, fill=myyellow!10, draw=black] (-5.8, -0.05) rectangle (5.8, 1.4);
    \node[font=\footnotesize\bfseries] at (0, 1.15) {引擎层 (Rust)};
    \node[sbox] at (-4.2, 0.6) {tmux Runtime};
    \node[sbox] at (-2.1, 0.6) {Memory Mgr};
    \node[sbox] at (0, 0.6) {Validate};
    \node[sbox] at (2.1, 0.6) {Instrum.};
    \node[sbox] at (4.2, 0.6) {Pipeline};

    % --- CLI 层 ---
    \draw[rounded corners, fill=myblue!10, draw=black] (-5.8, -1.55) rectangle (5.8, -0.25);
    \node[font=\footnotesize\bfseries] at (0, -0.5) {CLI / TUI 控制面};
    \node[sbox, minimum width=2.6cm] at (-3.75, -1.0) {\texttt{upctl init}};
    \node[sbox, minimum width=2.6cm] at (-1.15, -1.0) {\texttt{upctl deploy}};
    \node[sbox, minimum width=2.6cm] at (1.45, -1.0) {\texttt{upctl monitor}};
    \node[sbox, minimum width=2.6cm] at (4.05, -1.0) {\texttt{upctl compact}};
...
\begin{tikzpicture}[
  ...
  \node[svc, fill=green!15] (user) at (-6, 4.5) {用户浏览器};

  \draw[dashed, rounded corners, thick, gray] (-8, 3.5) rectangle (8, -2.5);
  \node[font=\tiny, gray] at (7.5, 3.2) {upctl-compose 容器网络};

  \node[svc, fill=blue!15] (nginx) at (-6, 2) {nginx};
  \node[svc, fill=green!15] (web) at (-3, 2) {upctl-web};
  \node[svc, fill=orange!15] (auth) at (-6, 0) {authcore};
  \node[svc, fill=purple!15] (svc) at (-2, 0) {upctl-svc};
  \node[svc, fill=teal!15] (gitea) at (2, 0) {Gitea};
  \node[svc, fill=yellow!15] (pg) at (-6, -2) {PostgreSQL};
  \node[svc, fill=red!15] (ai) at (2, -2) {ai-agent};
  \node[svc, fill=gray!15] (redis) at (6, 0) {redis};

  \node[draw, thick, rounded corners, fill=cyan!10, font=\tiny] (ds) at (2, -3.5) {DeepSeek API};

  \draw[->,thick] (user) -- (nginx);
  \draw[->,thick] (nginx) -- (web);
  ...
  \node[font=\tiny, right] at ([yshift=2pt]pg.east) {:5432};
  \node[font=\tiny, right] at ([yshift=2pt]redis.east) {:6379};
\end{tikzpicture}
\end{center}
...
\begin{frame}[shrink=10]{AI Agent 工作流程}
  \begin{center}
    \begin{tikzpicture}[node distance=0.3cm, auto, font=\tiny]
      ...
      \node[block, fill=purple!20, right=of ai] (svc) {upctl-svc};
      \node[block, fill=teal!20, right=of svc] (gitea) {Gitea};
      \node[block, fill=cyan!20, right=of gitea] (ds) {DeepSeek API};

      % Arrow 1: Poll
      \draw[->, thick] ([yshift=0.3cm]ai.east) -- node[above] {每 5m 轮询} ([yshift=0.3cm]svc.west);
      \draw[->, thick] ([yshift=0.3cm]svc.east) -- node[above] {查 approved 工单} ([yshift=0.3cm]gitea.west);
      \draw[->, thick, dashed] ([yshift=0.3cm]gitea.west) -- ([yshift=0.3cm]svc.east);
      \draw[->, thick, dashed] ([yshift=0.3cm]svc.west) -- ([yshift=0.3cm]ai.east);

      % Arrow 2: Process
      \draw[->, thick] ([yshift=-0.3cm]ai.east) -- node[below] {加 in\_progress} ([yshift=-0.3cm]svc.west);
      \draw[->, thick] ([yshift=-0.3cm]svc.east) -- node[below] {调用 API} ([yshift=-0.3cm]gitea.west);
      \draw[->, thick] ([yshift=-0.3cm]gitea.east) -- node[below] {发送工单} (ds);
      \draw[->, thick, dashed] (ds.south) ++(-0.3,0) -- ++(0,-0.5) -| node[below, xshift=-0.5cm] {处理结果} ([yshift=-1.2cm]ai.south) -- ([yshift=-0.6cm]ai.south);
    \end{tikzpicture}
  \end{center}
...
\begin{frame}[plain]
  \begin{center}
    ...
    \vspace{0.3cm}
    {\large \texttt{github.com/liweinan}}
